...
export const getJobFromSearchWithDescription = async ( desc, errorCb) => {
    try {
        const res = await axiosInstance.get(`positions.json?description=${desc}`);
        return res.data;
    } catch (err) {
        console.error(err);
        errorCb();
...
export const saveJobsIntoDBRecommendation = (jobs) => {
    console.log('save');
    jobs.forEach(job => {
        database.ref('recommendation/' + job.id).set({
            type: job.type,
            url: job.how_to_apply.match(/href="(.*?)"/)[1],
            created_at: job.created_at,
            company: job.company,
